...
mjolnir3_HELA <- function(lib, cores, remove_singletons = T, obipath = ""){
  ...
  system(paste0("sed 's/ seq_rank.*;//g' ",lib,"_new.fasta | sed 's/ merge.*count/;size/g' > ",lib,"_vsearch.fasta"),intern=T,wait=T)
