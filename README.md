...
mjolnir8_RAGNAROC() is the last step of the MJOLNIR pipeline. It requires a table of original sample names with the internal agnomes used along the rest of the pipeline. RAGNAROC also reorders the sample columns by alphabetical order and produces the .final_dataset.csv file. RAGNAROC will apply a total abundance filter to the MOTUs, removing MOTUs that has total abundance lower than a given value (default min_reads=2). It can also apply a Relative Read Abundance (RRA) filter on a sample by sample basis, removing reads of MOTUs that are below a relative RRA threshold (default min_relative=1/50000) for every particular sample. RAGNAROC will also optionally remove the MOTUs that have been assigned to prokaryotic sequences and the MOTUs that are considered contaminants (as specified in a dedicated text file). The MOTUs surviving RAGNAROC can be also ordered by their taxonomy or their abundance.
